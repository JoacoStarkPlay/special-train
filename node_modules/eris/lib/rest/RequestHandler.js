...
class RequestHandler {
    constructor(client, options) {
        ...
        this.options = options = Object.assign({
            ...
            requestTimeout: client.options.requestTimeout || (options.use_twilight ? 1000 * 60 : 15000)
        }, options);
        ...
        if (this.options.use_twilight) {
            NODE_REQUEST_LIB = HTTP
        }
    }
    ...
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const actualCall = (cb) => {
                ...
                let req;
                if (!this.options.use_twilight || finalURL.startsWith('/webhooks')) {
                    try {
                        req = HTTPS.request({
                            method: method,
                            host: 'discord.com',
                        ...
                        if(resp.statusCode >= 300) {
                            if(resp.statusCode === 429) {
                                ...
                                if (!route.includes('webhooks')) {
                                    this._client.emit('ratelimit-hit', { global: resp.headers['x-ratelimit-global'], info: {method, route, retryAfter, message: `${resp.headers["x-ratelimit-global"] ? "Global" : "Unexpected"} 429 (╯°□°）╯︵ ┻━┻: ${response}\n${content} ${now} ${route} ${resp.statusCode}: ${latency}ms (${this.latencyRef.latency}ms avg) | ${this.ratelimits[route].remaining}/${this.ratelimits[route].limit} left | Reset ${this.ratelimits[route].reset} (${this.ratelimits[route].reset - now}ms left)`} })
                                } else {
                                    this._client.emit('webhook-ratelimit-hit', {retryAfter: delay, webhookID: finalURL.split('/')[2], message: `${resp.headers["x-ratelimit-global"] ? "Global" : "Unexpected"} 429 (╯°□°）╯︵ ┻━┻: ${response}\n${content} ${now} ${route} ${resp.statusCode}: ${latency}ms (${this.latencyRef.latency}ms avg) | ${this.ratelimits[route].remaining}/${this.ratelimits[route].limit} left | Reset ${this.ratelimits[route].reset} (${this.ratelimits[route].reset - now}ms left)`})
                                }
                                if (resp.headers['x-ratelimit-global']) {
                                    this._client.emit('global-ratelimit-hit', this.ratelimits[route].reset - now)
                                }
